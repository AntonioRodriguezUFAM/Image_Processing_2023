...
// measure the execution time of any member function :


double Image::MeasureExecutionTime(double func) {
...
Image& Image::grayscale_avg_parallel() {
	...
	if (imageData == nullptr || imageSize == 0) {
		std::cout << "Empty image data." << std::endl;
		return *this;
	}
	...
	if (channels < 3) {
		...
		auto grayscaleFunc = [&]() {
			for (size_t index = 0; index < imageSize; ++index) {
				if (index % 3 == 0) {  // Check if it's a channel position
					size_t gray = (imageData[index] + imageData[index + 1] + imageData[index + 2]) / 3;
		...
		for (int i = 0; i < numThreads; ++i) {
			threads.emplace_back(grayscaleFunc);
		}
...
Image& Image::ImageMirror() {
	...
	for (int y = 0;y < h;++y) {
		// Loop through each pixel in the left half of the row
		for (int x = 0;x < w / 2;++x) {
			// Get pointers to the pixels on both sides of the row
			px1 = &data[(x + y * w) * channels]; // Original Pixel

			px2 = &data[((w - 1 - x) + y * w) * channels];

			// Step 1:  w - 1 - x calculates the horizontal index of the mirrored pixel.
			// Step 2:  w represents the width of the image, and subtracting x from w - 1 gives the index of the corresponding pixel on the right side of the row.
			// Step 3:  y * w calculates the starting index of the current row by multiplying the row number y with the width w. 
			//This ensures that the correct row is accessed.
			//  Step 4: (w - 1 - x) + y * w combines the horizontal and vertical indices to determine the overall index of the mirrored pixel in the image.


			//// Swap the pixel values using a temporary buffer
			//memcpy(tmp, px1, channels);
			//memcpy(px1, px2, channels);
			//memcpy(px2, tmp, channels);

			// Copy pixel values from left to right
		...
		// Fill the right half with white pixels
		for (int x = w / 2; x < w; ++x) {
			px2 = &data[(x + y * w) * channels];

			// Set pixel values to white
			for (int c = 0; c < channels; ++c) {
				px2[c] = 255;  // Set to maximum value for white
			}
		}
